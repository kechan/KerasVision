...
def identity_block(X, f, filters, stage, block):
    """
    Implementation of the identity block as defined in Figure 3 of Residual Networks Homework assignment
...
def convolutional_block(X, f, filters, stage, block, s = 2):
    ...
    X = Conv2D(F3, (1, 1), strides=(1, 1), name = conv_name_base+'2c', kernel_initializer=glorot_uniform(seed=SEED))(X)
    X = BatchNormalization(axis = 3, name = bn_name_base + '2c')(X)
...
def build_model(input_shape = (64, 64, 3), nb_classes=6, params=None):
    ...
    nb_classes -- integer, number of classes
    ...
    # Stage 1
    X = Conv2D(64, (7, 7), strides = (2, 2), name = 'conv1', kernel_initializer = glorot_uniform(seed=SEED))(X)
    ...
    # output layer
    X = Flatten()(X)
    X = Dense(nb_classes, activation='softmax', name='fc' + str(nb_classes), kernel_initializer = glorot_uniform(seed=SEED))(X)
