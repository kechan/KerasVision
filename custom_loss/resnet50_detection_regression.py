...
from model.convnet.detection import generate_conv_index_conv_dim
...
def obj_detection_loss_weighted_for_r(y_true, y_pred):
  ...
  matching_box_xy = matching_true_boxes[..., :2]
  y_pred_xy = t_pred[..., 1:3]
  xy_loss = K.sum(xy_scale * detectors_mask * K.square(matching_box_xy - y_pred_xy), axis=(-4, -3, -2, -1))

  matching_box_r = matching_true_boxes[..., 2:3]
  y_pred_r = t_pred[..., 3:4]
  r_loss = K.sum(r_scale * detectors_mask * K.square(matching_box_r - y_pred_r), axis=(-4, -3, -2, -1))
  ...
  total_loss = 0.5 * (confidence_loss + classification_loss + xy_loss + r_loss)
